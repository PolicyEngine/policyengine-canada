...
from policyengine_canada.tools.branched_simulation import BranchedSimulation
import numpy as np
...
def tax_unit_non_dep_sum(var, tax_unit, period):
    return tax_unit.sum(
        ...
        * not_(tax_unit.members("is_dependent", period))
    )
...
def sum_contained_tax_units(var, population, period):
    ...
    is_head = population.members("is_head", period)
