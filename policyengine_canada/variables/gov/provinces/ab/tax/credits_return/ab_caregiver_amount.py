...
class ab_caregiver_amount(Variable):
    ...
    def formula(person, period, parameters):
        ...
        is_dependant = (person("is_parent_or_grandparent", period) | (person("age", period) >= p.lower_age_eligibility & person("is_disabled", period)))
        dependant_net_income = person("individual_net_income", period) * is_dependant

        income_eligibility = dependant_net_income <= p.upper_dependant_income_threshold
        caregiver_amount = (p.upper_dependant_income_threshold - dependant_net_income) * income_eligibility
