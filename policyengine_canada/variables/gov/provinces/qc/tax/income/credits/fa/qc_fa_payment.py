...
class qc_fa_payment(Variable):
    ...
    def formula(household, period, parameters):
        ...
        maximum_child_amount = household.sum(
            p.child_amount.maximum * shared_custody_reduction
        )

        single_parent_max_amount = where(
            has_spouse, 0, p.single_parent_amount.maximum
        )
        ...
        minimum_child_amount = household.sum(
            p.child_amount.minimum * shared_custody_reduction
        )
        single_parent_min_amount = where(
            has_spouse, 0, p.single_parent_amount.minimum
        )
